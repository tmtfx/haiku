...
BString ExecuteLocalTool(const char* tool_name, const char* args_json) {
	BString name(tool_name);
	BString result;

	// Nota: Qui dovresti parsare 'args_json' se il tool richiede argomenti complessi.
	// Per adesso facciamo un check sui nomi dei tool disponibili:

	if (name == "get_system_info") {
		// Esempio Tool 1: Informazioni base sul sistema Haiku
		result = "Sistema Operativo: Haiku OS\n";
		result << "Uptime: " << RunSystemCommand("uptime");
	} 
	else if (name == "list_directory") {
		// Esempio Tool 2: Elenco file (puoi estrarre il path dal JSON se lo versi in un parser)
		// Ipotizziamo una semplice esecuzione di sicurezza per test
		result = RunSystemCommand("ls -la /boot/home");
	}
	else if (name == "run_command") {
		// Esempio Tool 3: Esecuzione generica (ATTENZIONE alla sicurezza/sanitizzazione!)
		// In una versione reale cercheresti il campo "cmd" dentro args_json
		result = "Esecuzione comandi disattivata per motivi di sicurezza o in attesa di parsing JSON.";
	} 
	else {
		// Fallback se il modello si inventa un tool inesistente
		result << "Errore: Il tool '" << tool_name << "' non è supportato da questo server.";
	}
