...
bool LoadAISettings(AISettings& out)
{
    ...
    // New: provider and base_url_override (allows editing base_url for remote plugins)
    if (m.FindString("provider", &s) == B_OK) out.provider = s;
...
bool SaveAISettings(const AISettings& settings)
{
    ...
    // New: persist provider and override flag
    m.AddString("provider", settings.provider.String());
