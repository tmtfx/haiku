...
//#include <x86gprintrin.h> // per il salvataggio registri xmm
...
x86_sha256_process(BCryptoRequest* request)
{
	dprintf("BCrypto - x86CPUDigest: process request %p, alg: %d, vectors: %ld\n", 
        request, request->algorithm, request->vectorCount);
    // 1. Validazione paranoica degli input
    if (request == NULL || request->algorithm != B_CRYPTO_SHA256) return B_BAD_VALUE;
    
    // Se non c'è una sorgente o una destinazione valida, esci subito
    if (request->vectorCount == 0 || request->source == NULL) return B_OK;
    if (request->destination == NULL) {
        dprintf("x86CPUDigest: ERROR - request->destination is NULL!\n");
        return B_BAD_VALUE;
    }
    
    dprintf("x86CPUDigest: dest[0].iov_base: %p\n", request->destination[0].iov_base);
    
    if (request->destination[0].iov_base == NULL) {
         dprintf("x86CPUDigest: ERROR - iov_base is NULL!\n");
         return B_BAD_VALUE;
    }
    ...
    for (size_t i = 0; i < request->vectorCount; i++) {
    	dprintf("x86CPUDigest: vector[%ld] base: %p, len: %ld\n", 
            i, request->source[i].iov_base, request->source[i].iov_len);
        const uint8* data = (const uint8*)request->source[i].iov_base;
        size_t len = request->source[i].iov_len;
        totalLen += len;
        ...
        if (i == request->vectorCount - 1) {
            ...
            memset(pad, 0, 128);
            memcpy(pad, data, len); // Copia il residuo < 64 byte
            
            pad[len] = 0x80;
            size_t padLen = (len < 56) ? 64 : 128;
            
            // Lunghezza in bit, Big Endian
