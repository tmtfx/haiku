...
x86_sha256_process(BCryptoRequest* request)
{
    ...
    /*uint32_t final_abcd[4] = {0x6a09e667, 0xbb67ae85, 0x3c6ef372, 0xa54ff53a};
    uint32_t final_efgh[4] = {0x510e527f, 0x9b05688c, 0x1f83d9ab, 0x5be0cd19};
    
    for(int i=0; i<4; i++) {
        res[i] += final_abcd[i];
        res[i+4] += final_efgh[i];
    }

    // 5. ESTRAZIONE BIG-ENDIAN (Sempre con lo swap 3-2-1-0)
    uint32_t* out = (uint32_t*)request->destination[0].iov_base;
    out[0] = __builtin_bswap32(res[3]); // A
    out[1] = __builtin_bswap32(res[2]); // B
    out[2] = __builtin_bswap32(res[1]); // C
    ...
    out[6] = __builtin_bswap32(res[5]); // G
    out[7] = __builtin_bswap32(res[4]); // H*/
    // 3. Output al sistema (Raddrizziamo l'ordine qui in C per chiarezza)
    /*uint32_t* destination = (uint32_t*)request->destination[0].iov_base;
    destination[0] = __builtin_bswap32(res[3]); // A
    destination[1] = __builtin_bswap32(res[2]); // B
    destination[2] = __builtin_bswap32(res[1]); // C
    destination[3] = __builtin_bswap32(res[0]); // D
    destination[4] = __builtin_bswap32(res[7]); // E
    destination[5] = __builtin_bswap32(res[6]); // F
    destination[6] = __builtin_bswap32(res[5]); // G
    destination[7] = __builtin_bswap32(res[4]); // H*/
    /*uint32_t* out = (uint32_t*)request->destination[0].iov_base;
    out[0] = 0xDEADBEEF;
    out[1] = 0xCAFEBABE;
    out[2] = 0xBAADF00D;
