...
inline uint32 GetEffectiveCursorOffset() {
...
#define CUR_CTRL    (INTEL_CURSOR_CONTROL + GetEffectiveCursorOffset())
#define CUR_BASE    (INTEL_CURSOR_BASE + GetEffectiveCursorOffset())
#define CUR_POS     (INTEL_CURSOR_POSITION + GetEffectiveCursorOffset())
#define CUR_PAL     (INTEL_CURSOR_PALETTE + GetEffectiveCursorOffset())
#define CUR_SIZE    (INTEL_CURSOR_SIZE + GetEffectiveCursorOffset())
	...
	uint8* andMask, uint8* xorMask)
{
	...
	if (gInfo->shared_info->device_type.Generation() == 5) {
		...
		write32(CUR_CTRL, CURSOR_ENABLED | gInfo->shared_info->cursor_format);
	}
