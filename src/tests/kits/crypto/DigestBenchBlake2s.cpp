...
int main(int argc, char** argv) {
    ...
    crypto.GetEngineName(B_CRYPTO_BLAKE2S, engineName, sizeof(engineName));
    
    printf("DEBUG: Indirizzo buffer digest (userspace): %p\n", digest);
    size_t hashLen = crypto.GetHashLength(B_CRYPTO_BLAKE2S);
    bigtime_t start, end;

    printf("Benchmark BLAKE2S su: %s (%.2f MB)\n", filename, sizeMB);
    ...
    start = system_time();
    status_t status = crypto.Digest(B_CRYPTO_BLAKE2S, &file, digest);
    end = system_time();
    ...
    if (fullBuffer == NULL) {
        ...
        start = system_time();
        status_t status = crypto.Digest(B_CRYPTO_BLAKE2S, fullBuffer, size, digest);
        end = system_time();
